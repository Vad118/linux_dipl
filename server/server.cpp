...
int _server::initialize()
{
    ...
    int a=1;
    setsockopt(ServerSocket,SOL_SOCKET,SO_REUSEADDR,&a,sizeof(int));
...
void _server::stop()
{
    ...
    for(int i=0;i<getTotalClients();i++)
    {
        ...
        close(disp->table[i].clientSocket);
    }
    //WSACleanup();
}
...
dispatcher_answer _server::receiveMessage(int client_id)
{
    ...
    tv.tv_sec = 0;
    tv.tv_usec = 0;
    int clientSocket=disp->table[client_id].clientSocket;
    //Очищаем readfds
    FD_ZERO(&readfds);
...
void _server::clientDisconnected(int client_id)
{
    ...
    if(disp->table[client_id].clientMonitoringSocket!=NULL)
        close(disp->table[client_id].clientMonitoringSocket);
