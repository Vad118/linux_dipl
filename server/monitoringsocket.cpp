...
int MonitoringSocket::initialize()
{
    ...
    int a=1;
    setsockopt(monitorSocket,SOL_SOCKET,SO_REUSEADDR,&a,sizeof(int));
    fcntl(monitorSocket, F_SETFL, O_NONBLOCK);
...
void MonitoringSocket::stop()
{
    ...
    for(int i=0;i<disp->nclients;i++)
        close(disp->table[i].clientMonitoringSocket);
    //WSACleanup();
}
...
receiveStruct MonitoringSocket::receiveMessage(int client_id)
{
    ...
    tv.tv_sec = 0;
    tv.tv_usec = 0;
    int clientSocket=disp->table[client_id].clientMonitoringSocket;
    //Очищаем readfds
    FD_ZERO(&readfds);
    ...
    if(FD_ISSET(clientSocket,&readfds))
    {
        ...
        char pBuff[1024];
        if((bytes_recv=recv(clientSocket,pBuff,sizeof( receiveStruct),0)) &&(bytes_recv!=0) && (bytes_recv!=-1))
        {
...
void MonitoringSocket::collectActorsAndTheirMessages()
{
    for(int i=0;i<disp->nclients;i++)
    {
        ...
        while(!finish)
        {
            ...
            tv.tv_sec = 0;
            tv.tv_usec = 0;
            int clientSocket=disp->table[i].clientMonitoringSocket;
            //Очищаем readfds
            FD_ZERO(&readfds);
            //Заносим дескриптор сокета в readfds
            FD_SET(clientSocket,&readfds);
            ...
            if(FD_ISSET(clientSocket,&readfds))
            {
                ...
                if((bytes_recv=recv(clientSocket,pBuff,sizeof( saveActor),0)) &&(bytes_recv!=0) && (bytes_recv!=-1))
                {
        ...
        if(count_client_messages>0)
        {
            ...
            while(!finish)
            {
                ...
                if(FD_ISSET(clientSocket,&readfds))
                {
                    ...
                    if((bytes_recv=recv(clientSocket,pBuff,sizeof( dispatcher_answer),0)) &&(bytes_recv!=0) && (bytes_recv!=-1))
                    {
...
void MonitoringCheckNewMultithread::check()
{
    ...
    //{
        ...
        //Последний параметр - время ожидания. Выставляем нули чтобы
        //Select не блокировал выполнение программы до смены состояния сокета
        select(monitoringSocketObj->monitorSocket+1,&readfds,NULL,NULL,&tv);
        //Если пришли данные на чтение то читаем
        int clientSocket;
        char buf[STR_SIZE];
